...
use crate::index_slicing::SliceConfig;
...
impl OpesSampler {
    ...
    pub fn new_without_tracking(num_samples: usize) -> Self {
        let mut config = OpesConfig::default();
        config.num_samples = num_samples;
        config.track_samples = false;
        Self::new(config)
    }
    ...
    pub fn sample_with_ttn<R: Rng>(
        ...
        debug!(
            ...
            samples.len(), self.stats.sampling_time_ms
        );
    ...
    fn sample_sequential<R: Rng>(
        ...
        self.stats.avg_acceptance_rate =
            samples.len() as f64 / attempts.max(1) as f64;
    ...
    fn sample_parallel<R: Rng>(
        ...
        let mut samples: Vec<OpesSample> = results
            .into_iter()
            .flatten()
            .collect();
        ...
        self.stats.avg_acceptance_rate =
            samples.len() as f64 / self.stats.attempts.max(1) as f64;
    ...
    fn estimate_partition_function<R: Rng>(
        ...
        for _ in 0..num_samples {
            let mut bits = vec![false; n_vars];
            for i in 0..n_vars {
                bits[i] = rng.random::<f64>() < 0.5;
            }
...
pub fn sample_low_energy_with_config<R: Rng>(
    ...
    let mut config = OpesConfig::default();
    config.num_samples = num_samples * 2;
    config.use_index_slicing = n_vars <= 20; // Only for small systems
...
impl MPO {
    ...
    pub fn contract_mpo_mpo(&self,
        other: &MPO,
        max_bond_dim: usize,
        svd_threshold: f64,
    ) -> MPO {
        assert_eq!(self.n_sites, other.n_sites, "MPOs must have same number of sites");
        ...
        for site in 0..n_sites {
            ...
            // This is simplified; actual MPO contraction is more complex
                    ...
                    for k in 0..bond_a_right {
                        for l in 0..bond_b_right {
                            for p in 0..phys_dim {
                                for q in 0..phys_dim {
                                    for r in 0..phys_dim {
                                        let idx_out = (i * bond_b_left + j,
                                                      k * bond_b_right + l,
                                                      p, q);
                                        result[idx_out] += a[[i, k, p, r]] * b[[j, l, r, q]];
                                    }
                                }
                            }
                        }
            ...
            // Truncate bond dimension if necessary
            let result = Self::truncate_tensor(&result,
                max_bond_dim,
                svd_threshold,
                phys_dim,
            );
    ...
    /// Truncate an MPO tensor using SVD.
    fn truncate_tensor(
        tensor: &Array4<f64>,
        max_bond_dim: usize,
        svd_threshold: f64,
        ...
        for i in 0..bond_left {
            for j in 0..bond_right {
                for p in 0..phys_dim {
                    for q in 0..phys_dim {
                        matrix[[i * bond_right + j, p * phys_dim + q]] =
                            tensor[[i, j, p, q]];
                    }
                }
            }
        ...
        // For simplicity, just truncate by taking a subarray
        ...
        for i in 0..new_bond_left {
            for j in 0..new_bond_right {
                for p in 0..phys_dim {
                    for q in 0..phys_dim {
...
pub fn sample_amplified_mpo<R: Rng>(
    ...
    let amp_result = spectral_amplification(hamiltonian, &amp_config);
    ...
    for _ in 0..num_candidates {
        ...
        // Compute "amplified energy" (simplified)
        let base_energy = hamiltonian.energy(&bits);
        // The amplification effectively raises the probability of low-energy states
...
mod tests {
    ...
    fn make_test_hamiltonian() -> CvpHamiltonian {
        ...
        CvpHamiltonian::new(&target, &b_cl, &basis_int, &basis_f64, &mu, &c)
    }
    ...
    fn test_index_to_bits() {
        assert_eq!(OpesSampler::index_to_bits(0, 4), vec![false, false, false, false]);
        assert_eq!(OpesSampler::index_to_bits(5, 4), vec![true, false, true, false]);
    }
    ...
    fn test_parallel_sampling() {
        ...
        let slice_config = SliceConfig::for_tnss(2);
        let samples = sample_parallel_with_slicing(&ham, 5, 2, &slice_config, &mut rng);
    ...
    fn test_mpo_mpo_contraction() {
        ...
        let result = mpo1.contract_mpo_mpo(&mpo2, 8, 1e-10);
    ...
    #[test]
    fn test_spectral_amplification() {
        let ham = make_test_hamiltonian();
        ...
        let result = spectral_amplification(&ham, &config);
    ...
    #[test]
    fn test_hybrid_amplification_sampling() {
        let ham = make_test_hamiltonian();
        let mut rng = ChaCha8Rng::seed_from_u64(42);

        let samples = sample_hybrid_amplification(&ham, 5, 4, true, &mut rng);

        assert!(!samples.is_empty());
        // Check sorted by energy
        for i in 1..samples.len() {
            assert!(samples[i].1 >= samples[i-1].1 - 1e-12);
        }
    }

    #[test]
    fn test_mpo_bond_dimension_tracking() {
        let mut rng = ChaCha8Rng::seed_from_u64(42);
        let mpo1 = MPO::random(4, 2, 4, &mut rng);
        let mpo2 = MPO::random(4, 2, 4, &mut rng);

        let result = mpo1.contract_mpo_mpo(&mpo2, 4, 1e-10);

        // Bond dimension should be bounded by max_bond_dim
        assert!(result.max_bond_dim <= 4);
    }
}
