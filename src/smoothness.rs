...
/// factorization of u. This relationship is used to derive congruences
/// for factorization.
#[derive(Clone, Debug)]
pub struct SrPair {
    ...
    /// `e_u[0]` is the sign bit (0 for positive, 1 for negative).
    /// `e_u[1..]` are prime exponents for each basis prime.
    pub e_u: Vec<u32>,
    /// Exponent vector of `w` over the smoothness basis.
    /// `e_w[0]` is the sign bit (0 for positive, 1 for negative).
    /// `e_w[1..]` are prime exponents for each basis prime.
    pub e_w: Vec<u32>,
}
...
pub fn factor_smooth(n: &Integer, basis: &SmoothnessBasis) -> Option<Vec<u32>> {
    ...
    for i in 0..basis.len() {
        let p_int = basis.get_int(i).unwrap();
...
mod tests {
    ...
    fn test_sr_pair_exponent_vector_reconstruction() {
        ...
        if let Some(pair) = pair {
            ...
            let u_reconstructed = reconstruct_from_exponents(&pair.e_u, &basis);
            assert_eq!(pair.u, u_reconstructed, "e_u should reconstruct u");

            // Verify e_w reconstructs w
            let w_reconstructed = reconstruct_from_exponents(&pair.e_w, &basis);
            assert_eq!(pair.w, w_reconstructed, "e_w should reconstruct w");
        }
    }

    /// Helper: Reconstruct an integer from its exponent vector
    fn reconstruct_from_exponents(exponents: &[u32], basis: &SmoothnessBasis) -> Integer {
        let mut result = Integer::from(1);
        let sign = if exponents[0] == 0 { 1 } else { -1 };

        for i in 1..exponents.len() {
            if exponents[i] > 0 {
                let p = basis.get(i - 1).unwrap();
                result *= Integer::from(p).pow(exponents[i]);
            }
        }
