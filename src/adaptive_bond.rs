...
impl PidParams {
    ...
    pub fn for_tnss(max_bond: usize) -> Self {
        Self {
            ...
            kd: 0.08,
            min_bond: 2,
            max_bond: max_bond.max(4).min(128),
...
impl PidState {
    ...
    pub fn has_converged(&self, threshold: f64) -> bool {
        ...
        let recent = &self.entropy_history[self.entropy_history.len() - 5..];
...
impl AdaptiveBondManager {
    ...
    pub fn update(&mut self, entropies: &[f64]) -> Vec<usize> {
        assert_eq!(
            entropies.len(),
            self.states.len(),
            "Entropy slice length must match number of bonds"
        );
    ...
    fn update_single_bond(
        ...
        // Compute target entropy based on current bond dimension
        let max_entropy = (state.current_bond as f64).ln();
        ...
        let max_integral = 100.0; // Prevent integral windup
    ...
    pub fn average_entropy(&self) -> f64 {
        self.states
            .iter()
            .filter_map(|s| s.entropy_history.last())
            .copied()
            .sum::<f64>()
...
impl AdaptiveBondManager {
    ...
    pub fn stats(&self) -> AdaptiveBondStats {
        ...
        AdaptiveBondStats {
            ...
            num_increases: 0, // Would need history tracking
            num_decreases: 0,
        }
    }
}
...
mod tests {
    ...
    fn test_pid_update_increases_bond() {
        ...
        let new_bonds = manager.update(&[high_entropy]);
    ...
    fn test_pid_respects_bounds() {
        ...
        let mut manager = AdaptiveBondManager::new(1, 2, params.clone());

        // Try to force below minimum
        for _ in 0..10 {
            manager.update(&[0.001]); // Very low entropy
        }
        ...
        for _ in 0..10 {
            manager2.update(&[100.0]); // Very high entropy
        }
    ...
    fn test_convergence_detection() {
        ...
        for _ in 0..10 {
            state.entropy_history.push(1.0);
        }
