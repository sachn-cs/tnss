...
use crate::{
    ...
    bkz::{progressive_bkz_reduce, bkz_reduce, BKZConfig},
    gf2_solver::kernel_basis,
    hamiltonian::CvpHamiltonian,
    index_slicing::SliceConfig,
    lattice::SchnorrLattice,
    smoothness::{try_build_sr_pair, SmoothnessBasis, SrPair},
    ttn::{TreeTensorNetwork, TTNConfig},
    Error, Result,
};
use log::{debug, info};
...
impl Config {
    ...
    pub fn default_for_bits(bits: usize) -> Self {
        ...
        // Heuristic: choose c so that max(last_row) ≈ max(f(j))
    ...
    pub fn ttn_config(&self) -> TTNConfig {
        TTNConfig {
            ...
            pid_params: self.adaptive_pid_params.clone(),
...
pub fn factorize(n: &Integer, cfg: &Config) -> Result<FactorResult> {
    ...
    info!("Factoring {}-bit semiprime {} with optimized pipeline", bits, n);
    info!("Configuration: adaptive_bonds={}, index_slicing={}, slices={}",
          cfg.enable_adaptive_bonds, cfg.enable_index_slicing, cfg.effective_slices());
    ...
    while cvp_count < cfg.max_cvp {
        ...
        let mut lattice = SchnorrLattice::new(cfg.n, n, cfg.c, &mut rng);
        stats.lattice_time_ms += lattice_start.elapsed().as_secs_f64() * 1000.0;

        // Stage 2: Perform lattice reduction
        ...
        if cfg.use_bkz {
            ...
            if cfg.bkz_progressive {
                ...
                let bkz_config = BKZConfig {
                    ...
                    segment_size: 32,
                    pruning_method: crate::pruning::PruningMethod::Auto,
                    num_tours: 10,
                    pruning_levels: 8,
        ...
        let samples = if cfg.use_ttn_sampler {
            sample_with_ttn(&hamiltonian, cfg, &mut rng)
        } else {
            sample_fallback(&hamiltonian, cfg.gamma, &mut rng)
        };
        ...
        if let Some(best_energy) = samples.iter().map(|(_, e)| *e).reduce(f64::min) {
            ...
            if improvement < cfg.convergence_threshold {
                ...
                if cfg.enable_early_termination && convergence_count >= 5 {
                    debug!("Early termination: converged for {} CVPs", convergence_count);
                }
            } else {
                convergence_count = 0;
            }
        ...
        // Process samples, potentially in parallel
        let sample_results: Vec<Option<SrPair>> = if cfg.enable_index_slicing && samples.len() > 100 {
            use rayon::prelude::*;
            samples
                .par_iter()
                .map(|(bits, _energy)| {
        ...
        for sr_opt in sample_results {
            if let Some(sr) = sr_opt {
                let key = (sr.u.to_string(), sr.w.to_string());
                if seen.insert(key) {
                    sr_pairs.push(sr);
                    found_this_cvp += 1;
                }
        ...
        debug!(
            ...
            cvp_count, found_this_cvp, sr_pairs.len(),
        ...
        if sr_pairs.len() >= needed_relations {
            ...
            let result = try_extract_factors_optimized(
                n, &sr_pairs, cfg.pi_2, cfg.combination_trials, &basis,
            );
            ...
            if let Some((p, q)) = result {
                ...
                stats.avg_bond_dim = if cfg.enable_adaptive_bonds {
                    Some(cfg.ttn_bond_dim as f64) // Would get actual average from adaptive manager
                } else {
                    None
                };

                info!("Factorization complete in {:.2}s", start_time.elapsed().as_secs_f64());
...
fn sample_with_ttn<R: Rng>(
    ...
    if cfg.enable_index_slicing && cfg.gamma > 50 {
        ...
        sample_low_energy_internal(&ttn, hamiltonian, cfg.gamma, rng)
    }
}
...
fn sample_with_index_slicing<R: Rng>(
    ...
    let n_vars = hamiltonian.n_vars();
    let slice_config = cfg.slice_config();
    ...
    // Sort by energy and return top gamma
...
fn process_sample(
    ...
    let e: Vec<i64> = (0..lattice.dimension)
        .map(|j| {
            ...
            let b_j = &point[j];
            Integer::from(b_j / f_j)
                .to_i64()
                .expect("f(j) must divide b_j exactly")
        })
        .collect();

    // Verify last coordinate consistency
    let last_coord_computed: Integer = e.iter()
        .enumerate()
...
/// Sample low-energy configurations using TTN.
...
fn extract_basis_representations(
    ...
    for col in 0..cols {
        ...
        for row in 0..dim {
            let v = &basis[col][row];
            col_int.push(v.to_i64().unwrap_or(0));
...
fn try_extract_factors_optimized(
    ...
    let matrix: Vec<Vec<u8>> = if cols > 100 {
        use rayon::prelude::*;
        (0..rows)
            .into_par_iter()
            .map(|i| {
        ...
        for (j, sr) in sr_pairs.iter().enumerate() {
            for i in 0..rows {
                matrix[i][j] = ((sr.e_w[i] + sr.e_u[i]) % 2) as u8;
            }
        }
        matrix
    };
    ...
    let try_basis_parallel = kernel.len() > 10;

    if try_basis_parallel {
        use rayon::prelude::*;
        let results: Vec<Option<(Integer, Integer)>> = kernel
            .par_iter()
            .map(|tau| try_tau_vector(n, tau, sr_pairs, pi_2, basis))
            .collect();
...
fn try_tau_vector(
    ...
    for i in 1..=pi_2 {
        ...
        let p_i = basis.get(i - 1).expect("Basis should have enough primes");
        let p_int = Integer::from(p_i);

        if k[i] > 0 {
            a *= p_int.pow(k[i] as u32);
        } else {
            b *= p_int.pow((-k[i]) as u32);
        }
    }
    ...
    let s_mod_n = Integer::from(&a * b_inv) % n;
