...
impl<R: Rng> SimulatedAnnealingSampler<R> {
    ...
    fn local_search_refine(&mut self, h: &CvpHamiltonian, state: &mut Vec<bool>, energy: &mut f64) {
        let n = h.n_vars();
        ...
        while improved {
            ...
            let order: Vec<usize> = (0..n).map(|i| (i, self.rng.random::<f64>())).collect::<Vec<_>>()
                .into_iter()
                .map(|(i, _)| i)
                .collect();
...
impl<R: Rng> Sampler for SimulatedAnnealingSampler<R> {
    fn sample(&mut self, h: &CvpHamiltonian, gamma: usize) -> Vec<Config> {
        ...
        results.sort_by(|a, b| {
            compare_energy(a.energy, b.energy)
                .expect("Invalid energy values in sort")
        });
...
impl BeamNode {
    ...
    fn new(energy: f64, bits: Vec<bool>) -> Option<Self> {
        ...
        // Scale and clamp to i64 range
        let scaled = (energy * ENERGY_SCALE)
...
impl Sampler for BeamSearchSampler {
    fn sample(&mut self, h: &CvpHamiltonian, gamma: usize) -> Vec<Config> {
        ...
        results.sort_by(|a, b| {
            compare_energy(a.energy, b.energy)
                .expect("Invalid energy in beam search results")
        });
...
mod tests {
    ...
    fn test_uniqueness() {
        ...
        for config in &results {
            assert!(unique.insert(config.bits.clone()), "Duplicate configuration found");
        }
    }
    ...
    fn test_edge_case_n1() {
        ...
        let h = CvpHamiltonian::new(&target, &b_cl, &basis_int, &basis_f64, &mu, &c);
    ...
    fn test_acceptance_probability() {
        ...
        assert!(prob2 > 0.99, "Should accept small uphill moves at high temp");
    }
