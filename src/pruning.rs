...
impl PruningConfig {
    ...
    pub fn discrete_pruning(n: usize) -> Self {
        Self {
            ...
            num_tours: 5,
            partition_type: PartitionType::Natural,
            pruning_levels: (n / 8).min(16).max(4),
            success_probability: 0.95,
            parallel: true,
...
fn extreme_pruning_enum(
    ...
    for tour in 0..config.num_tours {
        ...
        let aggression = config.pruning_aggressiveness * (tour + 1) as f64 / config.num_tours as f64;
        let radius_sq = gh_length * gh_length * (1.0 + aggression);

        trace!("Extreme pruning tour {}/{}: radius_sq={:.4}", tour + 1, config.num_tours, radius_sq);
        ...
        if best_norm_sq <= gh_length * gh_length * 1.1 {
            trace!("Extreme pruning: early termination after {} tours", tour + 1);
            break;
        }
    }
    ...
    PrunedEnumResult {
        short_vector: best_result.clone(),
...
fn discrete_pruning_enum(
    ...
    trace!("Discrete pruning: block_size={}, levels={}", block_size, config.pruning_levels);
    ...
    while let Some(node) = stack.pop() {
        ...
        if node.level == 0 {
            ...
            if node.coeffs.iter().any(|&c| c != 0) {
                ...
                if norm_sq < radius_sq {
                    if best_result.is_none() || norm_sq < best_result.as_ref().unwrap().1 {
                        best_result = Some((node.coeffs.clone(), norm_sq));
                    }
                }
            }
            continue;
        ...
        let center = if i + 1 < block_size {
            ...
            for j in (i + 1)..block_size {
                let mu = gso.gram_schmidt_coeffs[k + j].get(i).copied().unwrap_or(0.0);
        ...
        let min_c = (center - width).ceil() as i64;
        let max_c = (center + width).floor() as i64;

        // Push children in order
        for c in min_c..=max_c {
            let mut new_coeffs = node.coeffs.clone();
            new_coeffs[i] = c;
...
fn gaussian_heuristic(gso: &GsoData, k: usize, block_end: usize) -> f64 {
    ...
    let ball_vol_factor = (block_size / (2.0 * PI * std::f64::consts::E)).sqrt();
...
fn gamma(x: f64) -> f64 {
    ...
    // For half-integers like 2.5, 3.5, etc.
    if (x - 2.5).abs() < 1e-6 {
...
pub fn estimate_enum_cost(n: usize, pruning_factor: f64) -> usize {
    ...
    let n_f = n as f64;
    let base = PI * std::f64::consts::E / (2.0 * n_f);
...
mod tests {
    ...
    fn test_single_pruned_enum() {
        ...
        let (result, nodes) = single_pruned_enum(&gso, 0, 5, 10.0, 10000);
    ...
    #[test]
    fn test_extreme_pruning_small() {
        let gso = create_test_gso(20);
        let config = PruningConfig::extreme_pruning(20);

        let result = extreme_pruning_enum(&gso, 0, 20, &config);

        // Should complete
        assert!(result.stats.tours_completed > 0);
    }

    #[test]
    fn test_discrete_pruning_large() {
        // Use smaller dimension for test speed (discrete pruning still works for smaller dims)
        let gso = create_test_gso(20);
        let config = PruningConfig::discrete_pruning(20);

        let result = discrete_pruning_enum(&gso, 0, 20, &config);
    ...
    #[test]
    fn test_pruned_enumeration_auto() {
        let gso = create_test_gso(50);
        let config = PruningConfig::default();

        let result = pruned_enumeration(&gso, 0, 50, &config);

        // Should auto-select method and complete
        assert!(result.stats.nodes_visited > 0 || !result.stats.found_vector);
    }
}
