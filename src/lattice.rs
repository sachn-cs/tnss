...
impl SchnorrLattice {
    ...
    pub fn new<R: Rng>(
        ...
        debug_assert!(
            dimension >= 2,
            "lattice dimension must be at least 2"
        );
        debug_assert!(
        ...
        debug_assert!(
            *semiprime > Integer::ZERO,
            "semiprime must be positive"
        );
        ...
        trace!("  Generated {} primes: {} to {}", primes.len(), primes[0], primes.last().unwrap_or(&0));

        // Generate and shuffle diagonal weights
        let diagonal_weights = generate_diagonal_weights(dimension, rng);
        trace!("  Diagonal weights generated: {:?}", &diagonal_weights[..dimension.min(5)]);

        // Precompute scaling factor
        let scale = 10_f64.powf(scaling_param);
        debug_assert!(scale.is_finite() && scale > 0.0, "scale computation overflow");
        ...
        let last_row_values: Vec<i64> = primes
            ...
            .map(|&p| {
                ...
                log_val.round() as i64
            })
            .collect();
        trace!("  Last row computed: scale={:.3}, first value={}", scale, last_row_values[0]);
        ...
        target.extend(std::iter::repeat(0).take(dimension));

        // Compute ln(N) with fallback for large numbers
        let log_n = approximate_natural_log(semiprime);
        target.push((scale * log_n).round() as i64);
        trace!("  Target vector: ln(N) ≈ {:.3}, last entry={}", log_n, target.last().unwrap_or(&0));

        debug!(
            "Schnorr lattice constructed: {}x{} basis, target dim {}",
            dimension, dimension + 1, target.len()
        );
    ...
    pub fn verify_invariants(&self) -> bool {
        ...
        if cols != self.dimension || rows != self.dimension + 1 {
            warn!(
                ...
                cols, rows, self.dimension, self.dimension + 1
            );
            return false;
        }
        ...
        for j in 0..self.dimension {
            ...
            // Check diagonal matches diagonal_weights
            if diag != &Integer::from(self.diagonal_weights[j]) {
                warn!("Diagonal entry {} mismatch", j);
                return false;
            }

            // Check last row matches precomputed value
            let last_entry = &self.basis[j][self.dimension];
            if *last_entry != Integer::from(self.last_row_values[j]) {
                warn!("Last row entry {} mismatch", j);
                return false;
            }
        }

        // Verify target structure (zeros except last entry)
        for i in 0..self.dimension {
            if self.target[i] != 0 {
                warn!("Target[{}] should be 0, got {}", i, self.target[i]);
                return false;
            }
        }
...
/// # Arguments
///
/// * `x` - The integer to compute ln(x) for
...
mod tests {
    ...
    fn test_determinism() {
        ...
        for i in 0..5 {
            for j in 0..=5 {
                assert_eq!(lattice1.basis[i][j], lattice2.basis[i][j], "basis[{}][{}] mismatch", i, j);
            }
        }
    }
    ...
    fn test_last_row_correctness() {
        ...
        for j in 0..dimension {
            let expected = (scale * (lattice.primes[j] as f64).ln()).round() as i64;
            assert_eq!(lattice.last_row_values[j], expected, "Last row mismatch at index {}", j);
        }

        // Verify last_row_values matches basis entries
        for j in 0..dimension {
            let basis_last = lattice.basis[j][dimension].to_i64().unwrap();
            assert_eq!(lattice.last_row_values[j], basis_last, "Basis last row mismatch at {}", j);
        }
    }
    ...
    fn test_target_structure() {
        ...
        let scale = 10.0f64;
        let expected_last = (scale * 91f64.ln()).round() as i64;
    ...
    fn test_diagonal_weights_permutation() {
        ...
        assert_eq!(actual, expected, "diagonal_weights is not a valid permutation");
    }
    ...
    fn test_various_scaling_params() {
        ...
        for scaling_param in [0.5, 1.0, 1.5, 2.0].iter() {
            let lattice = SchnorrLattice::new(dimension, &semiprime, *scaling_param, &mut rng);
            assert!(lattice.verify_invariants(), "Invariants violated for scaling_param={}", scaling_param);
        }
    }
